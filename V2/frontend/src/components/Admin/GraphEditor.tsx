...
const GraphEditor: React.FC<GraphEditorProps> = ({ graphId = null, onClose }) => {
    ...
    const selectedNodeRef = useRef<cytoscape.NodeSingular | null>(null);
    ...
    const fetchGraph = async () => {
        try {
            ...
            const cleanedTabletCounts: Record<string, number> = {};
            if (optimalCount > 0) {
    ...
    useEffect(() => {
        ...
        const cy = cytoscape({
            ...
            elements: {
                ...
                edges: graphData.data.edges.map((edge: any) => ({
                    data: {
                        ...edge.data,
        ...
        cy.on('tap', 'node', (evt: any) => {
            ...
            if (selectedNodeRef.current) {
                if (selectedNodeRef.current === node) {
                    ...
                    const newEdge = {
                        data: {
                            id: `e${(selectedNodeRef.current as any).id()}-${(node as any).id()}`,
                            source: (selectedNodeRef.current as any).id(),
                            target: (node as any).id(),
                            controlPointDistance: 0
                        }
                    };
                    // Ajouter l'arête directement à Cytoscape
                    (cyRef.current as any).add({
                        data: {
                            id: newEdge.data.id,
        ...
        cy.on('cxttap', 'node, edge', (evt: any) => {
            ...
            if ((element as any).isNode()) {
                ...
                setGraphData((prev: any) => ({
                    ...prev,
                    data: {
                        nodes: prev.data.nodes.filter((node: any) => node.data.id !== (element as any).id()),
                        edges: prev.data.edges.filter((edge: any) =>
                            edge.data.source !== (element as any).id() && edge.data.target !== (element as any).id()
                        )
                    }
                }));
            } else if ((element as any).isEdge()) {
                ...
                setGraphData((prev: any) => ({
                    ...
                    data: {
                        ...prev.data,
                        edges: prev.data.edges.filter((edge: any) => edge.data.id !== (element as any).id())
                    }
                }));
            }
        });
    ...
    const resetGraph = () => {
        if (cyRef.current) {
            (cyRef.current as any).elements().remove();
        }
        setGraphData((prev: any) => ({
            ...prev,
            data: {
    ...
    const rearrangeGraph = () => {
        if (cyRef.current) {
            ...
            (cyRef.current as any).layout(layoutOptions).run();
            // Mettre à jour les positions dans l'état
            const nodes = (cyRef.current as any).nodes().map((node: any) => ({
                group: 'nodes',
    ...
    const toggleWorkshop = (workshopName: any): void => {
        setGraphData((prev: any) => ({
            ...prev,
            workshopData: {
                ...prev.workshopData,
    ...
    const updateWorkshopConfig = (workshopName: any, field: any, value: any): void => {
        setGraphData((prev: any) => {
            const newData = {
                ...prev,
                workshopData: {
            ...
            if (workshopName === 'coloring' && field === 'optimalCount') {
                ...
                const autoTabletCounts: Record<string, number> = {};
    ...
    const updateTabletCount = (color: any, count: any): void => {
    ...
    const saveNodePositions = () => {
        if (cyRef.current) {
            const updatedNodes = graphData.data.nodes.map((node: any) => {
                const cyNode = (cyRef.current as any).getElementById(node.data.id);
                if (cyNode && cyNode.length > 0) {
                    const position = (cyNode as any).position();
                    return {
                        ...node,
    ...
    const handleTabChange = (newTab: any): void => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (graphData.workshopData.coloring.enabled) {
                ...
                const invalidCounts = Object.entries(tabletCounts).filter(([_, count]: [string, any]) => 
            ...
            if (graphData.workshopData.coloring.enabled) {
                ...
                if (optimalCount >= 1) {
                    ...
                    const colorNames = ['Rouge', 'Bleu', 'Vert', 'Jaune', 'Orange', 'Violet', 'Rose', 'Vert lime', 'Gris foncé', 'Marron', 'Cyan clair', 'Orange vif', 'Vert néon', 'Bleu clair'];
                    const hexTabletCounts: Record<string, number> = {};
                    
                    Object.entries(graphData.workshopData.coloring.tabletCounts || {}).forEach(([colorName, count]: [string, any]) => {
                    ...
                    (cleanedData.workshopData as any).coloring = {
                        enabled: true,
            ...
            if (graphData.workshopData.spanningTree.enabled) {
                (cleanedData.workshopData as any).spanningTree = {
                    enabled: true
                };
            }
            ...
            if (graphData.workshopData.railwayMaze.enabled) {
                (cleanedData.workshopData as any).railwayMaze = {
                    enabled: true
                };
            }
                            ...
                            <div className="tab-content">
                                <div className="form-group">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setGraphData((prev: any) => ({ ...prev, name: e.target.value }))}
                                        required
                                    />
                                </div>
