...
const Defi: React.FC = () => {
    ...
    const [validationPopup, setValidationPopup] = useState<any>(null);
    const [isImpossibleEnabled, setIsImpossibleEnabled] = useState<boolean>(false);
    ...
    useEffect(() => {
        if (fetchedGraphs.length > 0) {
            ...
            coloringWorkshops.forEach((graph: any) => {
    ...
    useEffect(() => {
        if (selectedGraphData?.data) {
            ...
            graphConfig.data.nodes.forEach((node: any) => {
                if (node.position) {
                    node.position.y += 80;
                }
            });

            graphConfig.data.edges.forEach((edge: any) => {
                if (edge.data) {
    ...
    useEffect(() => {
        if (cyRef.current) {
            (cyRef.current as any).on('style', checkColoredPercentage);
            return () => {
                (cyRef.current as any).off('style', checkColoredPercentage);
            };
        }
    }, [cyRef.current]);
    ...
    function handleGraphSelect(event: any) {
    ...
    function checkColoredPercentage() {
        ...
        const totalNodes = (cyRef.current as any).nodes().filter((node: any) => !(node as any).data('isColorNode')).length;
        const coloredNodes = (cyRef.current as any).nodes().filter((node: any) => {
            return !(node as any).data('isColorNode') && (node as any).data('color') !== '#CCCCCC';
        }).length;
    ...
    function validateGraph() {
        ...
        (cyRef.current as any).nodes().forEach((node: any) => {
            if ((node as any).data('isColorNode')) return;

            const nodeColor = (node as any).data('color') || defaultColor;
            ...
            (node as any).connectedEdges().forEach((edge: any) => {
                const neighbor = edge.source().id() === (node as any).id() ? edge.target() : edge.source();
                if (!(neighbor as any).data('isColorNode') && (neighbor as any).data('color') === nodeColor) {
                    isValid = false;
                }
            });
        });
    ...
    function resetColors() {
        ...
        const unusedColors: Record<string, number> = {};
        const defaultColor = '#CCCCCC';
        ...
        (cyRef.current as any).nodes().forEach((node: any) => {
            if (!(node as any).data('isColorNode')) {
                ...
                (node as any).data('color', defaultColor);
            } else {
        ...
        (cyRef.current as any).nodes().filter((node: any) => (node as any).data('isColorNode')).forEach((node: any) => (cyRef.current as any).remove(node));

        let currentXPosition = 50;

        const allColors = { ...colorCounts };

        Object.entries(unusedColors).forEach(([color, count]: [string, any]) => {
            allColors[color] = (allColors[color] || 0) + count;
        });

        Object.entries(allColors).forEach(([color, count]: [string, any]) => {
    ...
    function handleImpossible() {
        ...
        const totalNodes = (cyRef.current as any).nodes().filter((node: any) => !(node as any).data('isColorNode')).length;
        const coloredNodes = (cyRef.current as any).nodes().filter((node: any) => {
            return !(node as any).data('isColorNode') && rgbToHex((node as any).style('background-color')) !== '#CCCCCC';
        }).length;
