...
*/
export const getDarkerColor = (hexColor: string, factor: number = 0.7): string => {
    // Remove # if present
...
*/
export const getLighterColor = (hexColor: string, factor: number = 0.3): string => {
    // Remove # if present
    const hex = hexColor.replace('#', '');
...
*/
export const findFreePositionX = (cy: cytoscape.Core): number | null => {
    const y = 50;
    let x = 50;
    const step = 50;
    const maxTries = 100;
    ...
    while (tries < maxTries) {
        const found = cy.nodes().some((node) => {
            return (node as any).position('x') === x && (node as any).position('y') === y && (node as any).data('isColorNode');
        });
        if (!found) return x;
        x += step;
        tries++;
