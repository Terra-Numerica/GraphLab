...
import '../../../styles/pages/ArbreCouvrant/PrimVisualization.css';

const PrimVisualization = ({ graph, cyRef }) => {
    ...
    useEffect(() => {
        ...
        const initializeSteps = () => {
            const newSteps = primAlgorithm(graph.data.nodes, graph.data.edges);
            setSteps(newSteps);
    ...
    useEffect(() => {
        ...
        // Color all edges up to currentStep
        if (currentStep >= 0) {
            for (let i = 0; i <= currentStep; i++) {
                ...
                // Colorer le sommet de départ si step.action === 'start'
                if (step.action === 'start' && step.node) {
                    const node = cyRef.current.getElementById(step.node);
                    if (node) {
                        node.addClass('prim-start')
                            .style({
